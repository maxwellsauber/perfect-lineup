const validateLineup = (lineup) => {
  ...
  return calculateSalary(lineup) <= 45000 &&
    ...
    evalPositions(allPositions, singlePositions, 1) &&
    evalPositions(allPositions, multiPositions, 3)
}
...
const evalPositions = (positions, requiredPositions, max) => {
  for (let i = 0; i < requiredPositions.length; i++) {
    if (getOccurrence(positions, requiredPositions[i]) !== max) return false
  }
